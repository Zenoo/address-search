 ...
 */ /** AddressSearch Class used to handle the AddressSearch module */var AddressSearch=/*#__PURE__*/function(){/**
...
var h,i=_createForOfIteratorHelper(a._onPredict);try{for(i.s();!(h=i.n()).done;){var l=h.value;l.call(a,a._input.value,a._predictions)}}catch(a){i.e(a)}finally{i.f()}}else if("ZERO_RESULTS"==c){var m=document.createElement("li");m.classList.add("empty-results"),m.innerText="No address found",a._predictions.appendChild(m),a._typeahead.value=""}else console.log(c)})}/**
     ...
     * @private
     */},{key:"_select",value:function _select(a){var b=this,c=!(1<arguments.length&&void 0!==arguments[1])||arguments[1];return new Promise(function(d,e){b._fetchPlace.getDetails({placeId:a,fields:b._apiFields,sessiontoken:b._token},function(a,f){if(f==google.maps.places.PlacesServiceStatus.OK){//onSelect callbacks
if(b._generateToken(),b._togglePredictions("off"),b._resetPredictions(),b.value=a,b._input.value=a.formatted_address,b._lure.value=b._input.value,b._typeahead.value="",Object.entries(b._components).forEach(function(a){var c=_slicedToArray(a,2),d=c[0],e=c[1],f=d.endsWith("_short")?b._getPlaceComponent(d.slice(0,-6),!0):b._getPlaceComponent(d);e.value=f,e.previousElementSibling.value=f,e.readOnly=!!e.value.length}),b._input.blur(),c){var g,h=_createForOfIteratorHelper(b._onSelect);try{for(h.s();!(g=h.n()).done;){var k=g.value;k.call(b,b.value)}}catch(a){h.e(a)}finally{h.f()}}d()}else{console.log(f);var i,j=_createForOfIteratorHelper(b._onError);try{for(j.s();!(i=j.n()).done;){var l=i.value;l.call(b,f)}}catch(a){j.e(a)}finally{j.f()}e(f)}})})}/**
     * Get a component for the current place.
     * @private
     */},{key:"_getPlaceComponent",value:function _getPlaceComponent(a,b){var c=this.value.address_components.filter(function(b){return b.types.includes(a)});return c.length?b?c[0].short_name:c[0].long_name:""}/**
     * Generate a new unique token for Google Places API
     * @private
     */},{key:"_generateToken",value:function _generateToken(){for(var a=this._token;""==a||this._usedTokens.includes(a);)a=(Math.random()+1).toString(36).substring(7);this._token=newToken,this._usedTokens.push(this._token)}/**
     ...
     */},{key:"onSelect",value:function onSelect(a){return this._errors.length||this._onSelect.push(a),this}/**
     * Removes every callback previously added with {@link AddressSearch#onSelect}
     * @returns {AddressSearch} The current {@link AddressSearch}
     */},{key:"offSelect",value:function offSelect(){return this._errors.length||(this._onSelect=[]),this}/**
     ...
     * @returns {AddressSearch} The current {@link AddressSearch}
     */},{key:"onPredict",value:function onPredict(a){return this._errors.length||this._onPredict.push(a),this}/**
     * Removes every callback previously added with {@link AddressSearch#onPredict}
     * @returns {AddressSearch} The current {@link AddressSearch}
     */},{key:"offPredict",value:function offPredict(){return this._errors.length||(this._onPredict=[]),this}/**
     ...
     * @returns {AddressSearch}   The current {@link AddressSearch}
     */},{key:"onError",value:function onError(a){return this._errors.length||this._onError.push(a),this}/**
     * Removes every callback previously added with {@link AddressSearch#onError}
     * @returns {AddressSearch} The current {@link AddressSearch}
     */},{key:"offError",value:function offError(){return this._errors.length||(this._onError=[]),this}/**
     * Manually sets the AddressSearch value
     * @param {String|PlaceResult} value New value
     * @param {Boolean} [triggerCallbacks=true] Should the method trigger the select callbacks?
     */},{key:"setValue",value:function setValue(a){var b=this,c=!(1<arguments.length&&void 0!==arguments[1])||arguments[1];this._errors.length||("string"==typeof a?this._fetchPredictions.getPlacePredictions({input:a},function(a,d){if(d==google.maps.places.PlacesServiceStatus.OK){var g=a[0];b._select(g.place_id,c)}else{console.log(d);var e,f=_createForOfIteratorHelper(b._onError);try{for(f.s();!(e=f.n()).done;){var h=e.value;h.call(b,d)}}catch(a){f.e(a)}finally{f.f()}}}):(this.value=a,this._input.value=a.formatted_address,this._lure.value=this._input.value,this._typeahead.value=""))}/**
     ...
     */},{key:"setPlace",value:function setPlace(a){var b=!(1<arguments.length&&void 0!==arguments[1])||arguments[1];return this._errors.length?Promise.resolve():this._select(a,b)}/**
     * Resets the AddressSearch
     * @returns {AddressSearch} The current {@link AddressSearch}
     */},{key:"reset",value:function reset(){return this._errors.length||(this.value={},this._input.value="",this._lure.value="",this._typeahead.value=""),this}/**
     * Refreshes the Google API fetchers
     * @returns {AddressSearch} The current {@link AddressSearch}
     */},{key:"refreshService",value:function refreshService(){return this._errors.length||(this._fetchPredictions=new google.maps.places.AutocompleteService,this._fetchPlace=new google.maps.places.PlacesService(document.createElement("div")),this._economizer={}),this}/**
     * Use a different 'google' object to fetch data
     * @param {Object} googleService - The service to use
     * @returns {AddressSearch} The current {@link AddressSearch}
     */},{key:"useService",value:function useService(a){return this._errors.length||(this._fetchPredictions=a.maps.places.AutocompleteService(),this._fetchPlace=a.maps.places.PlacesService(document.createElement("div")),this._economizer={}),this}/**
     * Manually set which fields should be returned by the Google Places API
     * @param {String[]} fieldList List of field names
     * @returns {AddressSearch} The current {@link AddressSearch}
     */},{key:"setFields",value:function setFields(a){return this._apiFields=a,this}/**
     * Removes any AddressSearch mutation from the DOM
     */},{key:"destroy",value:function destroy(){if(!this._errors.length){this._lure.id&&this._input.setAttribute("id",this._lure.id),this._lure.getAttribute("name")&&this._input.setAttribute("name",this._lure.getAttribute("name")),this._input.parentNode.parentNode.insertBefore(this._input,this._wrapper),this._wrapper.remove(),this._input.classList.remove("address-search-input"),document.querySelectorAll(".address-search-lure[data-refer-to=\""+this.uniqueId+"\"]").forEach(function(a){return a.remove()});//Remove event listeners
var a=this._input.cloneNode(!0);this._input.parentNode.replaceChild(a,this._input)}}/**
     * Removes any AddressSearch mutation from the DOM
     * @param {String} selector The AddressSearch input selector
     * @static
     */}],[{key:"destroy",value:function destroy(a){var b=document.querySelector(a);// If it exists
if(b){var c=b.nextElementSibling;// If it has been initialized as an AddressSearch
if(c&&c.matches(".address-search-input")){// Remove outside lures
...
var e=c.cloneNode(!0);c.parentNode.replaceChild(e,c)}}}}]),a}();
